...
public interface Field extends Serializable {
    ...
    public static Stream<Field> concat(
            Iterator<Field> left,
            Iterator<Field> right
    ) {
        return Stream.concat(
                Stream.iterate(left, Iterator::hasNext, UnaryOperator.identity()),
                Stream.iterate(right, Iterator::hasNext, UnaryOperator.identity()))
                .map(Iterator::next);
    }
}
